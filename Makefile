...
# GDC comes in gdc, gdc-15, gcc-gdc, gcc-gdc-15, and egdc (OpenBSD) flavors, so catch them all
DC_NAME       != basename "$(DC)"
OFLAG         != case "$(DC_NAME)" in *gdc*) echo "-o" ;; *) echo "-of=" ;; esac
...
UNITTEST_FLAG != case "$(DC_NAME)" in *gdc*) echo "-funittest" ;; *) echo "-unittest" ;; esac
