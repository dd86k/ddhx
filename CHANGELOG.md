...
- **Diff mode navigation**: new `diff-next` / `diff-prev` commands (bound to `}` / `{`) jump between
  differing regions, with a cursor mirror and better alignment in the diff panel.
- **Write protection overhaul**: new `-W`/`--writable` flag and `writemode` config; write mode is
  now enforced on fixed-size media, with a dedicated restricted-mode variable to back it.
- **Terminal rendering fixes**: partial rendering for readline, correct cell counting/truncation
  on narrow terminals, and a fixed CJK width heuristic. Tested on Linux (VTE, fbcon) and Windows
  (Terminal, conhost).
- **View/formatting fixes**: row drift when lines are truncated by terminal width is fixed.
...
- Terminal partial rendering for readline (63cdbf7), with fixes:
  - cell count no longer relies on terminalWrite, counts using graphs instead
  - text truncated correctly according to terminal size
  - scroll condition no longer ignores base position
  - CJK width heuristic no longer fails to fire (byte vs char comparison bug)

### Docs
- README updated (700136c)
- `ddhx.1` manpage updated for writemode, `-W`, and diff-next/diff-prev (6fd3d7e, 0ae096e, cb1c02d, 73a5983)

## v0.11.0

Small additions, fixes, and dropping "experimental" status. 🎉
...
Various additions like bookmarks, diff panel, inspector panel, and find-and-replace support.

Release tested on:

- Ubuntu 24.04 amd64 using VTE 0.76;
- Alpine 3.22 amd64 using framebuffer;
- Windows 11 amd64 using Windows Terminal 1.24.11321.0;
- Windows 10 amd64 using conhost;
- And NetBSD 10 amd64 using framebuffer.

### Bookmarks

Bookmarks. Anchors.

Bookmarks mark addresses (or ranges) you want to come back to. Set one with
`m` (`bookmark-toggle`) at the cursor, then jump between them with `[`
(`bookmark-prev`) and `]` (`bookmark-next`). Bookmarked bytes
are highlighted in both the data and text panels so they stay visible as you
scroll.

A bookmark can cover a single element, an explicit range, or the
current selection. The `bookmark-toggle` command picks the right action from
context, and `bookmark-set` / `bookmark-unset` commands are available for
custom bindings.

Use `bookmark-clear` to clear all bookmarks from memory, or before you load a
list from a file using `bookmark-load`. Want to save them? `bookmark-save`.

The `--bookmarks=FILE` switch allows loading from the CLI.

### Diff Panel

It is used to show byte-level differences against another file.

The diff panel can be shown using the `diff-file` FILE command and
can be hidden using `diff-file` (no arguments).

### Data Inspector Panel

The inspector is a small read-only panel that decodes the bytes under the
cursor as common numeric types (u8/i8, u16/i16, u32/i32, u64/i64, f32, f64)
all at once.

Toggle the panel with Alt+I, and flip between little- and big-endian
interpretation with Alt+E. Rows that don't have enough bytes available
(near EOF) show `N/A`; non-finite floats show `NaN` or `Inf`.

### Changelog

#### Additions

- Bookmarks! You can use `bookmark-toggle` command (mapped to m) to set and unset bookmarks
  ...
  - `bookmark-toggle` (`m`): Set a bookmark, or unset any that overlap the target range
  ...
  - `bookmark-load`: Load bookmarks from a file (additive, does not clear list)
  - `-B` and `--bookmarks=` to load bookmarks from the CLI
  - New color scheme: bookmark (feel free to color bookmark :red if you want!)
- Diff panel!
  - Use `diff-file FILE` to open panel to compare against FILE.
  - Use `diff-file` without any arguments to close it.
- `find-replace` and `find-replace-all` commands: Find and replace data using -- separator.
  - Syntax: `find-replace FROM -- WITH` (uses patterns)
  - Bound to Ctrl+R by default (^H used by most GUI editors is typically Backspace for terminals).
  - `redo` was default-remapped to Ctrl+Y (to be consistent with many other editors)
- `toggle-inspector` command (Alt+I): New inspector panel.
- `toggle-endian` command (Alt+E): (for inspector panel) toggle endianness.
- `replace-range` and `insert-range` commands: Replace or insert data using a range or selection.
  - These replace the behavior of replace and insert with an explicit range specifier.

#### Enhancements

- Make `find`, `find-next`, and `find-prev` force a selection over result when finding matching data.
  - Matches behaviour with most text editor offerings.

#### Fixes

- Fix using globbing near EOF would hit the update_progress assert.
- Fix tab autocomplete for commands.
- Fix color reverse video on conhost before Windows 10. Renderer gains ~0.5ms.
- Fix high-bit terminal drivers for Alt+key. This fixes Alt+I for OpenBSD and NetBSD terminal drivers.

#### Removals

- `replace`, `insert`: Removed length parameter, as it is assumed from given pattern or selection.
  - This allows behaviour like replace 0xff to replace byte at current cursor position.
  - Selection behaviour unchanged.
  - Older behavior with the length parameter moved to replace-range and insert-range commands.
- `redo` default binding reassigned to Ctrl+Y.
  ^Y is already the redo default for most text editors. This was a design oversight.
  You're still able to rebind the redo command.

## v0.9.3

- `find`: Add support for globbing with ? (single element) and * (one or more elements).
  e.g., `find x:FF ? FF` finds the next entry to [ 0xff, ?, 0xff ].
- Add `0o` as a pattern alias to `o:`, the octal prefix.
- `goto` command now understands the binary sequence with the 0b prefix.
- Add `TIOCGWINSZ` definition for OpenBSD.
- Respect VINTR (`c_cc[VINTR]` on POSIX platforms) and `AsciiChar == \003` on Windows for ^C (EXT).
  - Previous versions hardcoded the ^C check to Ctrl+C to cancel actions, which may be different in other keyboard layouts.
  - We're talking possibly Dvorak, Colemak, Workman, Neo, BÉPO, ЙЦУКЕН...
- Readline also allows Escape to cancel input.
- Readline no longer shouts "Cancelled" when cancelling input.

## v0.9.2

Small update with status bar customization and fixes.

### Status Customization

Three configurable format strings: `status-format`, `status-selection`, `status-report`.

Bars:
- `status-format`: The regular status bar.
  - Defaults to `%e %m | %t | %c | %8p`
- `status-selection`: Status bar when there is an active selection.
  - Defaults to `SEL: %V (%v Bytes)`
- `status-report`: Status message for the `report` (Ctrl+P) command.
  - Defaults to `%d / %s B (%q%%)`
...
|-----------|-------------|
| `%f` | Target file basename |
| `%F` | Full target path |
| `%s` | Document size in bytes (decimal) |
| `%S` | Document size in human-readable format (B, K, M, G, T, P) |
| `%P` | Document size in current address format |
| `%p` | Cursor position in current address format |
| `%d` | Cursor position in decimal |
| `%h` | Cursor position in hexadecimal |
| `%o` | Cursor position in octal |
| `%q` | Cursor position as percentage |
| `%v` | Selection length in bytes |
| `%V` | Selection range in current address format |
| `%e` | Edited indicator (`*` or space) |
| `%m` | Writing mode (OVR, INS, DIG, R/O) |
| `%c` | Character set |
| `%t` | Data type |
| `%%` | Literal `%` |
...
An optional width can be added between '%' and the specifier.

- `%10p` — right-align in 10-character field.
- `%-20f` — left-align in 20-character field.
- No padding added if the result is already wider than the specified width.
...
- Added status bar customization!
  - `status-format`, `status-selection`, `status-report` configurations.
  - Printf-like specifiers with support for width.

#### Enhancements

- Improved readline function that accepts accents on Windows and POSIX platforms.
  - On Windows, wide characters are translated to `CP_UTF8` to match output codepage.
  - On POSIX, input filtering was improved.

#### Fixes

- Add other possible values for "data" configuration, only affects `--help-config`.
- Add TIOCGWINSZ definition for NetBSD. ddhx works on NetBSD!
- Fix when turning off status bar would cause visual glitches.
- Fix selection in an empty file (zero-length) causing overflow.
- Fix "bind" command parameters in interactive mode.
- Fix capturing ctrl+return on POSIX platforms.

#### Removals

- Renamed `report-position` command to `report`.
  - The command was repurposed to perform just a form of customized user report.

## v0.9.1

Maintenance release focused around terminal enhancements.

### Changelog

#### Enhancements

- Added Makefile as an alternative to DUB.
- Command History: Previous commands are saved in memory, accessible via up/down keys in prompt.
- Input buffering, enabling scrolling with the mouse scrollwheel on POSIX platforms.
- Tab Completion: Commands can be autocompleted in the prompt using Tab.
- Windows scrollwheel mouse events are mapped to up/down arrow keys.

#### Fixes

- Fixed using scrollwheel on POSIX platforms caused insertion of 'A' and 'B' characters.

## v0.9.0

Version 0.9 brings new data modes, edit coalescing, a new digit cursor mode, various fixes and quality of life changes.

### PieceV3 & Coalescing

The new document editor revision, PieceV3, now has support for coalescing edits.

Instead of undoing every single edit one-by-one, if a series of edits were made next to each other, the series of edits are undone.

This is akin to modern text editors, where a word can be written, and a single undo removes the word.

To disable this feature, set configuration `coalesce` to "off". It only affects future edits.

### Data Modes

Data modes have been added: "x16", "x32", "d8", "d16", "d32", "o8", "o16", and "o32".

These provide new ways to represent data, and you can still directly
input existing and new elements into the document.

Example using "o32" and octal addresses (4 columns):
```
        oct            0           4          10          14                    
          0  04010020040 04010020040 04010020040 04010020040                  
         20  04010020040 04044313342 04010020040 22670420040      ...       ..
         40  22145561221 04044313342 04010020040 22670420040  .......       ..
         60  22145561221 04044313342 22145561040 34244313342  .......  .......
...
  OVR | o32 |    ascii |        0
```

For example, I managed to write an octal-encoded message from a screenshot to see its ASCII text data.

Set by the `data` configuration field, or via the command-line with `--data=MODE`.

### Digit Cursor

A new digit cursor mode was added, next to insert and overwrite.

This enables editing a single digit (or nibble), avoids having to retype
the entire element.

The new cycle is: Overwrite → Insert → Digit → Overwrite.

For example, if you have your cursor on the lower nibble of F0, and type 'f', it will register as FF, versus having to type "FF", or worse under x32: FFFFFFFF.

### Improved File Handling

In 0.8.3, when a file couldn't be opened as read-write, it would be confusing what other options were available, even if it would succeed with `-R`.

In 0.9, all files are opened as read-only. If an operating system error occurs, other than "file not found" (for a new buffer), it is reported (i.e., permission issue).

There are other fixes to make it possible for ddhx to save in network shares (tested GVFS with an SMB share, where `O_RDWR` and `chmod.2` are disallowed).

### Fast Save Optimization

When the operating system allows a file to be opened as read-write, only the dirty regions are written onto the file.

Otherwise, the fallback to traditional saving method is used (write and replace).

No more writing the entire 100 GiB file if a single byte was changed!

### Colors

ddhx 0.9 now has support for color mappings with the `color` configuration and command.

For example, to set a different background color to the cursor: `color cursor :blue` (blue background)

Here are available elements:
- `normal`: The default.
- `cursor`: The cursor.
- `selection`: The active selection.
- `mirror`: The mirrored cursor.
- `zero`: When data value is zero (affected by the `highlight-zeros` config).

Syntax: "foreground:background". Both are optional, empty meaning terminal default.

Colors:
- `default`: Default terminal color.
- `invert`: Invert color from default terminal color.
- Colors: `black`, `blue`, `green`, `aqua`, `red`, `purple`, `yellow`, `gray`, `lightgray`, `brightblue`, `brightgreen`, `brightaqua`, `brightred`, `brightpurple`, `brightyellow`, `white`.

### Changelog

#### Features

- "highlight-zeros" configuration.
- Add "x16", "x32", "d8", "d16", "d32", "o8", "o16", "o32" data modes.
- Digit cursor mode.
- Input coalescing with "piecev3" backend.
- Support for color schemes.

#### Enhancements

- Fast Save Optimization.
- Fallback to traditional saving method to help with network shares.
- Improve handling files (properly error out on permission errors).
- If opened as restricted, append "[readonly]" into the initial message.
- `address-spacing` now accepts negative values for right-alignment.

#### Fixes

- "save-as" command not working in read-only/restricted mode.
- Fixed opening and editing an empty file.
- Fixed `--addressing-spacing` calling charset config instead.
- On Windows, using conhost, re-hide the cursor on console resize.
- Fixed Backspace and Ctrl+Backspace being inverted on FreeBSD.

#### Removals

- `--readonly` was renamed to `--restrict` to better reflect its function. `-R` stays the same.
- Renamed confusing "addressing" configuration to "address", to be consistent with "data".
- The "autoresize" configuration is removed. Use "columns auto" instead, or Alt+R within the editor.

#### Misc.

- Command descriptions were revised for clarity.
- New ddhx.1 manpage.
...
- Misc: Ask to overwrite file for `export-range` command.
- Misc: Made save prompt easier to parse for its default action (`(Y/N)` to `[y/N]`).
...
- Deprecated: `autoresize` configuration, use `columns auto` instead.
- Removed: "chunk" backend.

## v0.8.1

Minor fixes relating to terminal functions.

### Changelog

- Improved: When a prompt is open, Escape does the same as Ctrl+C.
- Fixed: Unwanted newlines when a prompt is longer than 1 character
- Fixed: Scrolling with a prompt open would eventually crash it
...
I have written a
[blog post](https://blog.dd86k.space/post/939/remember-to-benchmark/)
...
- Insert 1,000 zeros at cursor position: `insert 1000 x:0`
- Fill the next 2 bytes with this integer: `replace 2 0x1234`
- Select the next 128 bytes: `select 128`
...
- Removed: `--help-debug`. Its content (target triple, compiler used) were moved into `--version`.

## v0.7.2

- Fixes editing after the first undo on a new buffer.
...
- `replace`/`insert` - add data from lazy patterns
- `replace-file`/`insert-file` - add data from files as references
...
I've also written a brief [blog article](https://blog.dd86k.space/post/866/ddhx-beyond-fixed-buffers/) about the technical details of the new backend.
...
- Development environment variables:
  - `DDHX_LOG` for debug logging.
  - `DDHX_BACKEND` for backend selection: "piece" (default) or "chunk".
  - `DDHX_CHUNKSIZE` for chunk backend buffer size (deprecated).
- Commands:
  - `report-version` - display ddhx version information.
  - `export-range` - export selection to a new file.
  - `select-all` (Ctrl+A) - select entire document.
  - `select-top` (Ctrl+Shift+Home) - select from current position to start.
  - `select-bottom` (Ctrl+Shift+End) - select from current position to end.
  - `select` - explicitly select a range using START and END parameters.
  - `delete` (Delete) and `delete-back` (Backspace) - remove bytes.
  - `insert` / `replace` - add data using patterns.
  - `insert-file` / `replace-file` - add data from files.
...
- Save prompt clarity: `"Save? (Yes/No/Cancel) "`.
- Patterns: Now supports multibyte values, so `0x1234` will be two bytes (platform-native).
- System path is now checked for the configuration file.
- The home path is now prioritized over the application path for the configuration file.
- Renamed `--help-configs` to `--help-config`, now displays which config is used.

**Removed:**
- `--log` and `--chunksize` command-line options (use environment variables instead).
...
- Windows: `SET DDHX_LOG=ddhx.log` (or environment variable dialog)
- POSIX: `export DDHX_LOG=ddhx.log` (or full path to your liking)
...
**Pattern operations:**
- `insert PATTERN` and `replace PATTERN` (with selection)
- Example: Select bytes, then `replace x:01 02 03 04`
- "Cut into" new patterns by overwriting partially
- Replace single bytes within inserted patterns

**File operations:**
- `insert-file PATH` and `replace-file PATH`
- Insert/replace at various positions
...
As version 0.5 is planned to have _editing capabilities_, so finally turning it into a "hex editor"!
...
- Bump version to 0.2.1 45ab6e22cce1a5c437f144d4e662cffdfb98467e
- Sort of searcher rewrite to be less crappy 4d2d6b16f64892d6907bb1660618a84a6716f97c
- Rename globals to make them a little more tolerable 73ccd4ba27fc99349e2d221dfba4dcf7a01dea02
...
### Full changelog
- Renderer tweaks 339d830a60ebca3828da68bf24599beea67cc095
- Re-write and enable new core renderer 5ee328a646f91cdf44a025704d086ec9fae96fb9
- Fixes 85b4034f459682d183cb26e9a49557e2e1bc3dfd
- windows: add scrolling support 4c4c77b263b8db171d39d05223e6fd14ae80e4d6
- Some unformat improvements b68039f3ebf55830d7c2f1f2d57471d60e7b9fdb
- Merge pull request #6 from TypicalFence/master 30317424ab62500dc018bb4109989fe5afe27109
- improve vimkeys switch 56b5ea484b18303e6733d1acf76848bedd194dfc
- open menu with colon 7dc3119e73e5deae38db87b5ae522136ac5a904a
- add vim keys 61e4df336e68ea77059da22a6983b58253063f66
- Code cleanup eca0d4866b463ddbeb6983a3a26d4a3e2bb8dbb9
- Fix 64-bit compilation 4d9fe06511440fe1560ef7ccb62f939ce6783537
- import cleaning 41f0d119a848a0016d015ae001f82930e39ae8eb
- File renames 97b0612e66e278e89ba217b06f837e460926a345
- Add #2 (Github) d8e53d9dedd2329ed6be4761284cfdcc627ead70
- Renderer optimization, tweaks, and README 523322748f93e070a7fba3df65eea4a6a6f82670
- Various improvements 091743bf1825af895a306eddc1bb474d481a0411
- searcher and menu small improvements 3ca4168ad7ce88eb766b6b3c854146326f017e8c
- Tweak CLI, redo text render, cleanup+optimizations, use mmfile 770ed49f615ef936ef897cafaca972e680b2b9b8
- README update 6d0eeb022a997ce79c4f49327d6428e1ded12e57
- Fix typo, missing '{' 488998ec4a794a82088dd2e307dcad8c6c031266
- Convert spaces to tabs, modified style 67c728ce178542094a062d3e270cd02fc7f05f7b
- Fix debugs in ddcon 5581085cbd2f2c453893f8ecfd01db9c48a6b29f
- Tweak ddcon 94d2f1501939a7b93fa4a0ec4cd853667d272c15
- gitignore 3286b1ef5412de9a166361cfa1696449899c9b67

## v0.1.0

Been a long while I have not touched this project, so I decided to take care of that.

In fact, 0xdd was my first tool published to Github and ddhx is its replacement, so it's only normal I'd work on it at some point again!

### Changelog
- Added relative offset jumping, e.g. `g +100h`
- Added new shortcut to goto: g
- Huge displayrenderer optimization and other minor optimizations
- Tweaked chunk searching size to 4096
- Tweaked the CLI
- Removed display mode entirely
- Removed `--width` and `--offset`. `-w` and `-o` are still available
